...
const FACTURE_STATUS_CONFIG: Record<string, { label: string; color: string; icon: React.ReactNode }> = {
  emise: { label: 'Émise', color: 'bg-warning/10 text-warning border-warning/30', icon: <Clock className="h-4 w-4" /> },
  envoyee: { label: 'Envoyée', color: 'bg-primary/10 text-primary border-primary/30', icon: <FileText className="h-4 w-4" /> },
  payee: { label: 'Payée', color: 'bg-success/10 text-success border-success/30', icon: <CheckCircle className="h-4 w-4" /> },
  retard: { label: 'En Retard', color: 'bg-destructive/10 text-destructive border-destructive/30', icon: <AlertCircle className="h-4 w-4" /> },
};
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground">
                  {format(parseISO(facture.date_facture), 'dd MMMM yyyy', { locale: fr })}
                </p>
              </div>
            </div>
        ...
        <div className="space-y-6 mt-4">
          ...
          {isConsolidated && (
            <div className="flex items-center gap-2 p-3 rounded-lg bg-primary/5 border border-primary/20">
              ...
              <div>
                <p className="font-medium text-sm">Facture Consolidée</p>
                <p className="text-xs text-muted-foreground">
                  Regroupe {facture.bls_inclus?.length} livraisons: {facture.bls_inclus?.join(', ')}
                </p>
              </div>
            </div>
          )}
            ...
            <CardContent>
              <div className="space-y-3">
                ...
                <div className="flex justify-between items-center py-2 border-b">
                  <div>
                    <p className="font-medium">Béton {facture.formule_id}</p>
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                  {facture.cur_reel !== null && (
                    <div className="text-center p-3 rounded-lg bg-muted/50">
                      <p className="text-xs text-muted-foreground mb-1">CUR Réel</p>
