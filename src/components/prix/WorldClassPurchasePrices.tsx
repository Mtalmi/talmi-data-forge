...
const T = {
  ...
  blue: '#D4A843',
  blueDim: 'rgba(212,168,67,0.12)',
  purple: '#9CA3AF',
  purpleDim: 'rgba(156,163,175,0.12)',
  gray: '#64748B',
...
function SectionHeader({ icon: Icon, title, badges, subtitle }: {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 0, marginBottom: 20, flexWrap: 'wrap' }}>
      ...
      {badges?.map((b, i) => (
        <span key={i} style={{
          fontFamily: "ui-monospace, monospace", fontSize: 12, fontWeight: 700, padding: '3px 10px',
          borderRadius: 100, background: b.bg, color: b.color, marginLeft: 8,
  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'flex', alignItems: 'baseline', gap: 4 }}>
        <span style={{ fontFamily: "ui-monospace, SFMono-Regular, 'SF Mono', Menlo, monospace", fontSize: 36, fontWeight: 700, color: '#fff', lineHeight: 1, letterSpacing: '-0.02em' }}>
  ...
  return (
    <div style={{ fontFamily: SANS, minHeight: '100vh', background: T.bg, color: T.textPri }}>
      ...
      <PageHeader
        icon={Package}
        title="Prix d'Achat"
        subtitle="Suivi des prix fournisseurs et tendances"
        ...
        actions={
          <>
            <button
              onClick={() => {
                const headers = 'Article;Fournisseur;Catégorie;Prix Actuel;Prix Précédent;Variation (%);Dernière MAJ';
                const csvRows = PRICE_ROWS.map(r => `${r.article};${r.fournisseur};${r.cat};${r.prix};${r.prev};${r.variation > 0 ? '+' : ''}${r.variation};${r.maj}`).join('\n');
              ...
              style={{
                ...
                cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 7, transition: 'all 0.15s ease',
              }}
            ><Download size={13} /> Exporter</button>
            <button
              onMouseEnter={() => setHoverUpdate(true)} onMouseLeave={() => setHoverUpdate(false)}
              style={{
                background: hoverUpdate ? '#FFE44D' : '#D4A843', color: '#0B1120', border: 'none', borderRadius: 8,
                padding: '7px 16px', fontFamily: 'DM Sans, sans-serif', fontWeight: 700, fontSize: 12,
                cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 7, transition: 'all 0.15s ease',
              }}
