-- Fix create_bl_from_bc: bons_commande.volume_restant is a generated column and must not be updated
...
BEGIN
  ...
  SET
    ...
    statut = CASE
      WHEN (COALESCE(volume_livre, 0) + p_volume_m3) >= volume_m3 THEN 'termine'
      ELSE 'en_production'
    END,
    updated_at = now()
  WHERE bc_id = p_bc_id;
