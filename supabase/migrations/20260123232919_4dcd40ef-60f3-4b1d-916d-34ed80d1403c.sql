...
-- Fix cancel_devis_approval to get full_name from profiles table instead of user_roles_v2
CREATE OR REPLACE FUNCTION public.cancel_devis_approval(p_devis_id text, p_reason text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  -- Get user name from profiles table (FIXED: was incorrectly querying user_roles_v2)
